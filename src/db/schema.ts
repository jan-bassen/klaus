...
export const costService = pgEnum("cost_service", [
	"tts",
	"embed",
	"stt",
	"llm",
...
export const tasks = pgTable(
	...
	(t) => [index("idx_tasks_chat").on(t.chatId)],
);
...
export const budgets = pgTable(
	"budgets",
	{
		...
		currentDailyUsd: numeric("current_daily_usd", {
			precision: 10,
			scale: 6,
		}).default("0"),
		currentMonthlyUsd: numeric("current_monthly_usd", {
			precision: 10,
			scale: 6,
		}).default("0"),
		createdAt: timestamp("created_at", { withTimezone: true })
	...
	(t) => [unique("uq_budgets_chat").on(t.chatId)],
);
...
export const reactions = pgTable(
	...
	(t) => [
		...
		unique("uq_reactions_sender_msg").on(
			t.chatId,
			t.messageExternalId,
			t.senderId,
		),
...
export const costs = pgTable(
	...
	(t) => [index("idx_costs_created").on(t.createdAt)],
);
...
export const invocations = pgTable(
	...
	(t) => [index("idx_invocations_message").on(t.messageId)],
);
