...
export const QUERIES: Record<string, QueryFn> = {
  recent_messages: async (params) => {
    ...
    return db.select({ id: messages.id, role: messages.role, content: messages.content, createdAt: messages.createdAt })
      .from(messages)
  ...
  active_tasks: async (params) => {
    ...
    return db.select().from(tasks).where(filter).orderBy(desc(tasks.createdAt)).limit(50);
  },
  ...
  file_list: async (params) => {
    ...
    return db.select({ id: files.id, path: files.path, mimeType: files.mimeType, sizeBytes: files.sizeBytes, createdAt: files.createdAt })
      .from(files)
