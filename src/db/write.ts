...
export async function embedText(text: string): Promise<number[]> {
	...
	db.insert(costs)
		.values({ service: "embed", units: tokens, costUsd: String(costUsd) })
		.catch((err) =>
			log.warn("[cost] failed to record embed cost", {
...
function splitBody(body: string): string[] {
	...
	for (const para of paragraphs) {
		...
		if (
			current &&
			estimateTokens(candidate) > config.chunking.thresholdTokens
		) {
			result.push(current);
			current = para;
		} else {
...
export async function writeNode(insert: NodeInsert): Promise<Node> {
	...
	const [node] = await db
		...
		.values({
			...insert,
			embedding,
			searchTsv: tsvectorExpr(insert.title, insert.body),
			tokenCount,
		})
		.returning();
...
export async function resolveQuotedMessageId(
	...
	const [row] = await db
		...
		.from(messages)
		.where(
			and(eq(messages.chatId, chatId), eq(messages.externalId, externalId)),
		)
		.limit(1);
	return row?.id ?? null;
}
