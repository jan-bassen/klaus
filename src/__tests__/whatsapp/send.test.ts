...
const { setSocket, enqueueMessage, drainQueue } = await import('@/whatsapp/send');

const mockSendMessage = mock(async (_jid: string, _content: unknown, _opts?: unknown) => ({
  key: { id: 'wa-msg-id-1' },
}));
...
beforeEach(() => {
  ...
  mockSendMessage.mockImplementation(async () => ({ key: { id: `wa-${crypto.randomUUID()}` } }));
  setSocket({ sendMessage: mockSendMessage } as any);
});
...
describe('enqueueMessage', () => {
  ...
  test('sends failure notification after max retries exhausted', async () => {
    mockSendMessage.mockImplementation(async (_jid: string, content: unknown) => {
      if (typeof (content as any)?.text === 'string' && (content as any).text.includes('Failed to deliver')) {
  ...
  test('routes image buffer as image content', async () => {
    ...
    const sentContent = mockSendMessage.mock.calls[0]![1] as any;
  ...
  test('passes quoted message key when quoted is set', async () => {
    ...
    const sendOpts = mockSendMessage.mock.calls[0]![2] as any;
