...
import type { TurnContext, AgentDefinition } from '@/types';
...
describeDb('memory tools', () => {
  test('memory.write creates a node', async () => {
    const result = await memoryWriteTool.execute(
      { type: 'topic', title: 'Test Node', body: 'Some content', tags: ['test'] },
      makeContext(),
    );
  ...
  test('memory.read returns a written node', async () => {
    ...
    const nodeId = writeResult.match(/Created node (.+)/)?.[1]!;
  ...
  test('memory.link creates an edge between two nodes', async () => {
    const r1 = (await memoryWriteTool.execute({ type: 'topic', title: 'A' }, makeContext())) as string;
    const r2 = (await memoryWriteTool.execute({ type: 'topic', title: 'B' }, makeContext())) as string;
    const idA = r1.match(/Created node (.+)/)?.[1]!;
    const idB = r2.match(/Created node (.+)/)?.[1]!;
  ...
  test('memory.link is idempotent (no error on duplicate)', async () => {
    ...
    await memoryLinkTool.execute({ sourceId: idA, targetId: idB, relation: 'related_to' }, makeContext());
    await memoryLinkTool.execute({ sourceId: idA, targetId: idB, relation: 'related_to' }, makeContext());
  ...
  test('memory.unlink removes an edge', async () => {
    ...
    await memoryLinkTool.execute({ sourceId: idA, targetId: idB, relation: 'related_to' }, makeContext());
    await memoryUnlinkTool.execute({ sourceId: idA, targetId: idB, relation: 'related_to' }, makeContext());
  ...
  test('memory.traverse walks a graph A→B→C with depth 2', async () => {
    const rA = (await memoryWriteTool.execute({ type: 'topic', title: 'A', body: 'node A' }, makeContext())) as string;
    const rB = (await memoryWriteTool.execute({ type: 'topic', title: 'B', body: 'node B' }, makeContext())) as string;
    const rC = (await memoryWriteTool.execute({ type: 'topic', title: 'C', body: 'node C' }, makeContext())) as string;
    const idA = rA.match(/Created node (.+)/)?.[1]!;
    const idB = rB.match(/Created node (.+)/)?.[1]!;
    const idC = rC.match(/Created node (.+)/)?.[1]!;

    await memoryLinkTool.execute({ sourceId: idA, targetId: idB, relation: 'related_to' }, makeContext());
    await memoryLinkTool.execute({ sourceId: idB, targetId: idC, relation: 'related_to' }, makeContext());
