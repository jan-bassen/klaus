...
const mockDispatch = mock(async (): Promise<string | undefined> => 'task-uuid-1');
mock.module('@/core/dispatch', () => ({ dispatch: mockDispatch }));

const mockCancel = mock(async () => {});
mock.module('@/core/queue', () => ({ getQueue: () => ({ cancel: mockCancel }) }));
...
const mockSelectFromWhere = mock((): any => ({
...
const { dispatchTool, taskCancelTool, taskListTool } = await import('@/tools/sets/task');
...
beforeEach(() => {
  ...
  mockSelectFromWhere.mockImplementation((): any => ({
    orderBy: mock(() => ({ limit: mock(async () => []) })),
    then: (resolve: any, reject?: any) => Promise.resolve([]).then(resolve, reject),
  }));
...
describe('dispatchTool', () => {
  test('async mode returns dispatch result with task ID', async () => {
    ...
    const opts = (mockDispatch.mock.calls[0] as any[])[0];
...
describe('taskCancelTool', () => {
  test('cancels a pending task', async () => {
    mockSelectFromWhere.mockImplementation(async () => [{ id: 'task-1', status: 'pending' }]);
    const result = await taskCancelTool.execute({ taskId: 'task-1' }, makeContext());
    expect(result).toContain('Cancelled');
    expect(mockUpdateSetWhere).toHaveBeenCalled();
  });

  test('returns not found for missing task', async () => {
    mockSelectFromWhere.mockImplementation(async () => []);
    const result = await taskCancelTool.execute({ taskId: 'missing' }, makeContext());
    expect(result).toContain('not found');
  });

  test('returns already-terminal for done task', async () => {
    mockSelectFromWhere.mockImplementation(async () => [{ id: 'task-1', status: 'done' }]);
    const result = await taskCancelTool.execute({ taskId: 'task-1' }, makeContext());
