...
describe("graphContextQuery", () => {
		...
		mockHybridSearch.mockImplementationOnce(async () => [
			{
				...
				score: 0.8,
				matchingChunk: "the relevant excerpt",
			},
		]);
	...
	test("multiple results are joined with double newline", async () => {
		mockHybridSearch.mockImplementationOnce(async () => [
			{
				node: fakeNode({ id: "n1", title: "Alpha", body: "alpha body" }),
				score: 0.9,
			},
			{
				node: fakeNode({ id: "n2", title: "Beta", body: "beta body" }),
				score: 0.7,
			},
		]);
	...
	test("tokenCount is non-zero when content present", async () => {
		mockHybridSearch.mockImplementationOnce(async () => [
			{
				node: fakeNode({ id: "n1", title: "X", body: "some content here" }),
				score: 0.8,
			},
		]);
