...
export const todo = (label: string) =>
	(it.todo as unknown as (label: string) => void)(label);
...
export const describeDb: typeof describe = DB_AVAILABLE
	? describe
	: describe.skip;
...
const DB_URL =
	...
	"postgres://postgres:postgres@localhost:5433/klaus_test";
...
async function startPostgres(): Promise<void> {
	try {
		...
		throw new Error(
			"Failed to start postgres via docker compose — is Docker running?",
		);
	}
}
