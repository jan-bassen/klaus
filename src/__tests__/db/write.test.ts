...
import { chunks, nodes, nodeVersions } from "@/db/schema";
...
describeDb("writeNode", () => {
	...
	test("chunks body that exceeds the token threshold", async () => {
		expect(estimateTokens(LONG_BODY)).toBeGreaterThan(
			config.chunking.thresholdTokens,
		);
		...
		const chunkRows = await db
			.select()
			.from(chunks)
			.where(eq(chunks.nodeId, node.id));
