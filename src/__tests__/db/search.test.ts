...
describeDb("hybridSearch", () => {
	...
	test("filters by tags when provided", async () => {
		...
		expect(ids).not.toContain(
			(await writeNode({ type: "assertion", body: "js" })).id,
		);
	});
	...
	test("expands related nodes via edges when expandEdges is true", async () => {
		const nodeA = await writeNode({
			type: "topic",
			title: "database systems",
			body: "database info",
		});
		const nodeB = await writeNode({
			type: "assertion",
	...
	test("each result has a positive score", async () => {
		await writeNode({
			type: "assertion",
			body: "score verification test content",
		});
