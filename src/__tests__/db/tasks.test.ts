...
describeDb("activeTasksQuery", () => {
	...
	test("done and failed tasks are excluded", async () => {
		await db
			.insert(tasks)
			.values({ chatId: CHAT_ID, objective: "Finished task", status: "done" });
		await db
			.insert(tasks)
			.values({ chatId: CHAT_ID, objective: "Broken task", status: "failed" });
		const result = await activeTasksQuery.run(turn);
		expect(result.content).toBe("");
		expect(result.tokenCount).toBe(0);
	});
	...
	test("multiple active tasks are each on their own line", async () => {
		await db
			.insert(tasks)
			.values({ chatId: CHAT_ID, objective: "Task A", status: "pending" });
		await db
			.insert(tasks)
			.values({ chatId: CHAT_ID, objective: "Task B", status: "running" });
		const result = await activeTasksQuery.run(turn);
	...
	test("tokenCount is non-zero when tasks are present", async () => {
		await db
			.insert(tasks)
			.values({ chatId: CHAT_ID, objective: "Some work", status: "pending" });
		const result = await activeTasksQuery.run(turn);
