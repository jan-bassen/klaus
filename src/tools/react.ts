...
const reactSchema = z.object({
  emoji: z.string().describe('Emoji to react with (e.g. "👍"). Pass an empty string to remove the reaction.'),
  messageRef: z.string().optional().describe('Message label from conversation history (e.g. "3") or "current". Defaults to the current message.'),
});

export const reactTool: ToolDefinition<typeof reactSchema> = {
  name: 'react',
  description: 'React to a message with an emoji. Use for lightweight acknowledgements — e.g. 👍 to confirm, ✅ on task done, ❤️ for appreciation. Pass "" to remove. Use messageRef to react to an older message from the conversation history.',
  ...
  execute: async ({ emoji, messageRef }, context) => {
    ...
    if (result instanceof Error) {
      log.warn('[react] sendReaction failed', { chatId, error: result.message });
