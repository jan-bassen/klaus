...
const vaultReadSchema = z.object({
  path: z.string().describe('Relative path to the note, e.g. "Projects/Klaus.md"'),
});

export const vaultReadTool: ToolDefinition<typeof vaultReadSchema> = {
  name: 'vault.read',
  description: 'Read a note from the Obsidian vault by its relative path. Returns the full markdown content including frontmatter.',
...
const vaultSearchSchema = z.object({
  query: z.string().describe('Search terms (case-insensitive substring match across all notes)'),
  limit: z.number().optional().default(10).describe('Max results to return'),
});

export const vaultSearchTool: ToolDefinition<typeof vaultSearchSchema> = {
  name: 'vault.search',
  description: 'Full-text search across all markdown notes in the Obsidian vault. Returns matching file paths with context lines.',
...
const vaultListSchema = z.object({
  directory: z.string().optional().default('').describe('Relative directory path, e.g. "Projects" or "" for root'),
  depth: z.number().optional().default(2).describe('How many levels deep to list (max 5)'),
});

export const vaultListTool: ToolDefinition<typeof vaultListSchema> = {
  name: 'vault.list',
  description: 'Browse the directory structure of the Obsidian vault. Returns a tree of folders and .md files.',
  ...
  execute: async ({ directory, depth: rawDepth }, _context) => {
    ...
    async function walk(dir: string, indent: number, remaining: number): Promise<number> {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          remaining--;
          remaining = await walk(path.join(dir, entry.name), indent + 1, remaining);
...
const vaultWriteSchema = z.object({
  path: z.string().describe('Relative path including .md extension, e.g. "Projects/New Note.md"'),
  content: z.string().describe('Full markdown content of the note (including frontmatter if desired)'),
});

export const vaultWriteTool: ToolDefinition<typeof vaultWriteSchema> = {
  name: 'vault.write',
  description: 'Create or overwrite a note in the Obsidian vault. Parent directories are created automatically.',
...
export const vaultAppendTool: ToolDefinition<typeof vaultAppendSchema> = {
  name: 'vault.append',
  description: 'Append content to an existing note (useful for daily notes, logs, inboxes). Creates the file if it does not exist.',
...
const vaultBacklinksSchema = z.object({
  noteName: z.string().describe('Note name without .md extension, e.g. "Klaus"'),
});

export const vaultBacklinksTool: ToolDefinition<typeof vaultBacklinksSchema> = {
  name: 'vault.backlinks',
  description: 'Find all notes that link to a given note via [[wikilinks]]. Returns file paths with the linking line.',
  inputSchema: vaultBacklinksSchema,
  execute: async ({ noteName }, _context) => {
    const glob = new Bun.Glob('**/*.md');
    const pattern = new RegExp(`\\[\\[${noteName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(\\|[^\\]]*)?\\]\\]`, 'i');
    const results: string[] = [];
...
export const vaultToolset: ToolsetDefinition = {
  name: 'vault',
  description: 'Use when you need to read, search, write, or browse notes in the Obsidian vault.',
  tools: [vaultReadTool, vaultSearchTool, vaultListTool, vaultWriteTool, vaultAppendTool, vaultBacklinksTool],
};
