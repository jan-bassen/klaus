...
const filesUploadSchema = z.object({
	...
	nodeId: z
		...
		.uuid()
		.optional()
		.describe("Optional graph node to link this file to"),
});

export const filesUploadTool: ToolDefinition<typeof filesUploadSchema> = {
	name: "files.upload",
	description:
		"Upload a file to the files volume and create a metadata row in the files table. Optionally links to a graph node via nodeId.",
...
export const filesDownloadTool: ToolDefinition<typeof filesDownloadSchema> = {
	name: "files.download",
	description:
		"Download a file from the files volume by UUID or partial filename. Returns base64-encoded content.",
	...
	execute: async ({ name }, _context) => {
		...
		const rows = isUuid
			...
			: await db
					.select()
					.from(files)
					.where(like(files.path, `%${name}%`));

		const [row] = rows;
		if (!row) return `No file found for: ${name}`;
...
export const filesListTool: ToolDefinition<typeof filesListSchema> = {
	name: "files.list",
	description:
		"List files by querying the files metadata table. Optionally filter by name prefix.",
	...
	execute: async ({ prefix }, _context) => {
		...
		return rows
			.map(
				(r) =>
					`${r.id} | ${path.basename(r.path)} | ${r.mimeType} | ${r.sizeBytes}B | ${r.createdAt.toISOString()}`,
			)
			.join("\n");
	},
	kind: "builtin",
...
export const filesDeleteTool: ToolDefinition<typeof filesDeleteSchema> = {
	name: "files.delete",
	description:
		"Delete a file — removes both the blob from the files volume and its metadata row.",
