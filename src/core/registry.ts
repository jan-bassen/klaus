 ...
 */
export function generateMetaTool(ts: ToolsetDefinition): ToolDefinition<typeof emptySchema> {
  const toolList = ts.tools.map((t) => `• ${t.name}: ${t.description}`).join('\n');
  return {
    name: `use_${ts.name}`,
...
function isToolsetDefinition(x: unknown): x is ToolsetDefinition {
  return (
    ...
    'tools' in x && Array.isArray((x as Record<string, unknown>).tools) &&
    !('execute' in x)
  );
}
