 ...
 */
export async function loadContextQueries(contextDir: string): Promise<ContextQuery[]> {
  const queries: ContextQuery[] = [];
  const glob = new Bun.Glob('*.ts');
  for await (const file of glob.scan({ cwd: contextDir })) {
    try {
      const mod = (await import(`${contextDir}/${file}`)) as Record<string, unknown>;
      for (const exported of Object.values(mod)) {
        if (isContextQuery(exported)) {
          queries.push(exported);
          log.debug('[assemble] loaded context query', { name: exported.name, file });
        }
      }
    } catch (err) {
...
function isContextQuery(x: unknown): x is ContextQuery {
  return (
    ...
    'priority' in x && typeof (x as Record<string, unknown>).priority === 'number' &&
    'run' in x && typeof (x as Record<string, unknown>).run === 'function'
  );
}
