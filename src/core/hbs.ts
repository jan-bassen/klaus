...
// -- Logic --
hbs.registerHelper('and', (...args: unknown[]) => (args.slice(0, -1) as unknown[]).every(Boolean));
hbs.registerHelper('or',  (...args: unknown[]) => (args.slice(0, -1) as unknown[]).some(Boolean));
hbs.registerHelper('not', (a: unknown) => !a);

// -- Utility --
hbs.registerHelper('default', (val: unknown, fallback: unknown) => val || fallback);
