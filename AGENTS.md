...
- `/abort` only cancels active runs; `/pause` and `/resume` control future schedules/timers; `/stop` is the combined panic path.
- Timers are stored as ISO instants; any user-visible listing should format them with `settings.locale` and `settings.timezone`.
- The entire repo `vault/` tree (agents, snippets, skills, templates, `settings.yml`, `overrides.yml`) is the first-run template only — the shipped defaults a fresh install starts with. At runtime Klaus reads the user's live `{vault}/Klaus/` directly and never merges repo defaults back in. So changing bundled defaults means editing the repo template; changing a running instance means editing the live vault. For `settings.yml` specifically, Zod validates with no `.default()` fallbacks, so a new setting needs editing both the schema in `src/infra/config.ts` and the template.
