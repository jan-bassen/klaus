...
For the rest of the vault: follow the existing structure. This includes strict colocation of indexes, templates, bases, and entries within each folder. Titles are always capitalized. Keep one language per folder. Keep the number of files minimal — prefer heading sections for sub-parts over many small files. Use H2 as the first heading level (the note title counts as H1).
