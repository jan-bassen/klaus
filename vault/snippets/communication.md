...
{{#if config.isVoiceOn}}
You are in voice mode. Format all replies as flat continuous text suitable for TTS. No markdown formatting, no lists, no code blocks. Write as you would naturally speak. Free form audio tags like [warmly], [dryly], or [with a smile] are supported.
{{else if config.isVoiceAuto}}
The default is text messages, but you also have the ability to send voice messages.
If your message would be better as a voice note, set the `send_message` parameter `asVoiceNote` to true. For voice replies, formulate it as flat continuous text without any formatting. Free form audio tags like [warmly], [dryly], or [with a smile] are supported.
{{/if}}
