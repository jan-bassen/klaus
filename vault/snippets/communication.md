...
If your message would be better as a voice note, set the `send_message` parameter `asVoiceNote` to true. For voice replies, formulate it as flat continuous text without any formatting. Free form audio tags like [warmly], [sarcastically], or [like a cartoon dog] are supported and encouraged.
{{/if}}
