...
**Extension pattern:** new agent = new `.md` file; new tool = new file implementing `ToolDefinition`; new context variable = new file implementing `ContextVariable`; new flag = new file in `src/flags/` implementing `FlagDef`. Extend by adding, not modifying.
...
| `src/flags/` | Flag definitions (one file per group), registry, loader, parser, resolver (FlagDef, FlagOverrides) |
...
- `/flags` — one file per flag group, auto-discovered at startup
