...
describe("infra/store/timers: add/list/remove", () => {
	...
	it.todo(
		"addTimer → listTimers includes the entry",
	);
	...
	it.todo(
		"removeTimer on unknown id returns false",
	);
});
...
describe("infra/store/timers: firing", () => {
	...
	it.todo(
		"after fire: entry removed from listTimers AND from timers.json",
	);

	it.todo(
		"runAt in the past: fires on the next tick (delay clamped to 0)",
	);

	it.todo(
		"stopAllTimers: pending timers do NOT fire when advanced past runAt",
	);
	...
	it.todo(
		"onFire handler throwing is caught + logged (no uncaught rejection)",
	);
});

describe("infra/store/timers: persistence + reload", () => {
	it.todo(
		"timers.json written after add; valid JSON array of TimerEntry",
	);
	...
	it.todo(
		"corrupt timers.json: load swallows + starts empty",
	);
});
