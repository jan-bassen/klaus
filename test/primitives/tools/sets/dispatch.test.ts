...
describe("dispatch tool: inline path", () => {
	...
	it.todo(
		"agent defaults to 'dispatch' when omitted",
	);

	it.todo(
		"explicit agent name: passed through",
	);

	it.todo(
		"returns dispatchFn's joined string (or 'done' when it's empty)",
	);

	it.todo(
		"propagates overrides list to dispatchFn",
	);

	it.todo(
		"trigger === {kind:'dispatch', parentRunId: turn.runId}",
	);
});

describe("dispatch tool: timer path (`when` set)", () => {
	it.todo(
		"when === '2h': creates timer runAt ≈ now + 2h (via parseRunAt)",
	);

	it.todo(
		"when === ISO datetime: passes through as the exact runAt",
	);
	...
	it.todo(
		"overrides propagate to timer.overrides when non-empty",
	);

	it.todo(
		"returns 'Timer set for @<agent> at <iso> [<id>]'",
	);

	it.todo(
		"invalid `when` string: parseRunAt throws",
	);
});
...
describe("dispatch tool: simulate handler", () => {
	...
	it.todo(
		"inline + sim: returns '(sim) done' when collector empty",
	);

	it.todo(
		"timer + sim: NO addTimer call; returns descriptive (sim) string",
	);
});

describe("dispatch.schedule", () => {
	it.todo(
		"execute: calls addSchedule with generated id + supplied fields",
	);
...
describe("dispatch.list", () => {
	it.todo(
		"execute: merges schedules + timers into a human-readable render",
	);
...
describe("dispatch.cancel", () => {
	...
	it.todo(
		"simulate + real id: adds to overlay.cancelledIds (disk untouched)",
	);

	it.todo(
		"unknown id: returns 'No schedule or timer found with ID …'",
	);
});

describe("parseRunAt", () => {
	it.todo(
		"'30m' → now + 30*60*1000 (±tolerance)",
	);
	...
	it.todo(
		"ISO datetime string → same date, toISOString()",
	);

	it.todo(
		"garbage input → throws with message mentioning valid forms",
	);
});
