...
1. The active tool list is serialised to JSON Schema, and server tools are appended verbatim. When `toolChoice` is `none`, only `send_message` is offered.
...
The reply is the concatenation of every accepted `send_message` text. Traces (steps, tool calls, results) are persisted to history unless the turn is ghosted. The `send_message` calls themselves are dropped from the stored trace, since the message is already recorded as its own row.
